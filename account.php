<?php include('header.php');?>

<div class="row">
    <div class="col-12">

        <div class="col-xl-12 col-md-6 col-12 user-profile-card">
           
                <div class="text-center mx-auto pt-3">
                    <div class="avatar bg-rgba-primary p-50">
                        <img class="img-fluid" src="app-assets/images/home/user-08.jpg" alt="img placeholder" height="134" width="134">
                    </div>
                </div>
                <div class="text-center">
                    <h4 style="color:#fff;">test9</h4>
                   
                    <!-- <p class="px-1">Jelly beans halvah cake chocolate gummies.</p> -->
                    <div class="d-flex justify-content-around mb-1">
                        <div class="card-icons d-flex flex-column">
                            <div class="row">
                              <div class="col-6">
                                <img class="img-fluid" src="app-assets/images/home/gp.PNG" height="24" width="24">
                              </div>
                              <div class="col-6" style="left: -30px;">
                                <span style="color:#e8c661;">7533.00</span>
                              </div>
                            </div>
                            <p style="color:#4a4a4b;text-align: center;font-size: 10px;">Wallet Balance</p>
                        </div>
                       
                        <div class="card-icons d-flex flex-column">
                           <div class="row">
                              <div class="col-6">
                                <img class="img-fluid" src="app-assets/images/home/gp.PNG" height="24" width="24">
                              </div>
                              <div class="col-6" style="left: -30px;">
                                <span style="color:#fff;">7533.00</span>
                              </div>
                            </div>
                            <p style="color:#4a4a4b;text-align: center;font-size: 10px;">Total Play</p>

                        </div>
                    </div>
                   
                </div>
           
        </div>

    </div>

    <div class="row">
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>

    </div>

     <div class="row">
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>
      <div class="col-3 outer-line">
        <div class="inner-line">My Downline</div>
      </div>

    </div>



</div>

<?php include('footer.php');?>
