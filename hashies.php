<?php 
$title = 'Hashies';
$dropdown = true;
include('header.php');
?>
<div class="card">
   <div class="card-body">
    <div data-tabbed-content id="TRX">
        
   
    <div class="row">
        <div class="col-12 hashies-wrapper">
                    <div class="text-center">

                        <div class="row">
                            <div class="col-12 text-center hashies-timer-top">
                                <h3><img class="img-fluid" src="app-assets/images/home/gp.png" height="24" width="24">7928.00  <span class="test-9">test 9</span></h3>
                            </div>

                            
                        </div>
                        <p style="color:#bacede;text-align: center;font-size: 16px;"><img class="img-fluid eos-img" src="app-assets/images/home/red-buble.png"><i class="game-type">TRX-TRON</i></p>

                        <h1 class="game-code" style="color:#fff;">43210974</h1>

                        <p style="text-align: center;"><span class="prev-block">Previous Block</span></p>
                        <p style="text-align: center;"><span class="prev-block">00000000029358deee026eedle21f1erdgrdgfrttg</span></p>

                       
                        <div class="d-flex justify-content-around mb-1 hashies-timer">
                            <div class="card-icons d-flex flex-column">
                                <div class="row">
                                  <div class="col-12">
                                    <h2 class="hashies-time">00:12</h2>
                                    <span class="hashies-text">Rounds Ends In</span>
                                  </div>
                                  
                                </div>
                                
                            </div>
                           <span class="divider">|</span>
                            <div class="card-icons d-flex flex-column">
                               <div class="row">
                                  <div class="col-12">
                                      <h2 class="hashies-time">00:42</h2>
                                      <span class="hashies-text">Next Round In</span>

                                  </div>
                                  
                                </div>
                               
                            </div>
                        </div>
                       

                       
                        <h3 class="gameid"><i>GAME ID #634217</i></h3>

                        <p style="text-align: center;"><span class="hashies-block">Hashies Block</span> <span class="hashies-block-text">434217</span></p>

                         <div class="row">
                            <div class="col-7">
                                 
                                  <div class="wrapper"> 
                                       <input class="trans-input" type="text">
                                    </div>
                            </div>
                            
                              <div class="col-5 text-right green-yellow-btn-wrapper">
                                <a href="myplay.php">
                                    <div class="tab-submit-yellow">
                                        <span class="unskewed">PLAY</span>
                                        <img src="app-assets/images/home/next.png" width="12">
                                       
                                    </div>

                               </a>
                             </div>
                           
                         </div>
                     

                    </div>

        </div>

    </div>

    <table class="table zero-configuration py-2 play">
        <thead>
            <tr>
                <th>Name</th>

            </tr>
        </thead>
        <tbody>

          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                    <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>




                                               
            </tbody>
            <tfoot>
                <tr>
                    <th>Name</th>
                </tr>
            </tfoot>
     </table>
    </div>


    <!---EOS start------>
     <div data-tabbed-content id="EOS" class="hidden">
        
   
    <div class="row">
        <div class="col-12 hashies-wrapper">
                    <div class="text-center">

                        <div class="row">
                            <div class="col-12 text-center hashies-timer-top">
                                <h3><img class="img-fluid" src="app-assets/images/home/gp.png" height="24" width="24">7928.00  <span class="test-9">test 9</span></h3>
                            </div>

                            
                        </div>
                        <p style="color:#bacede;text-align: center;font-size: 16px;"><img class="img-fluid eos-img" src="app-assets/images/home/red-buble.png"><i class="game-type">EOS-TRON</i></p>

                        <h1 style="color:#fff;">43210975</h1>

                        <p style="text-align: center;"><span class="prev-block">Previous Block</span></p>
                        <p style="text-align: center;"><span class="prev-block">00000000029358deee026eedle21f1erdgrdgfrttg</span></p>

                       
                        <div class="d-flex justify-content-around mb-1 hashies-timer">
                            <div class="card-icons d-flex flex-column">
                                <div class="row">
                                  <div class="col-12">
                                    <h2 class="hashies-time">00:17</h2>
                                    <span class="hashies-text">Rounds Ends In</span>
                                  </div>
                                  
                                </div>
                                
                            </div>
                           <span class="divider">|</span>
                            <div class="card-icons d-flex flex-column">
                               <div class="row">
                                  <div class="col-12">
                                      <h2 class="hashies-time">00:32</h2>
                                      <span class="hashies-text">Next Round In</span>

                                  </div>
                                  
                                </div>
                               
                            </div>
                        </div>
                       

                       
                       <h3 class="gameid"><i>GAME ID #634217</i></h3>

                        <p style="text-align: center;"><span class="hashies-block">Hashies Block</span> <span class="hashies-block-text">434217</span></p>

                         <div class="row">
                            <div class="col-7">
                                 
                                  <div class="wrapper"> 
                                       <input class="trans-input" type="text">
                                    </div>
                            </div>
                            
                              <div class="col-5 text-right green-yellow-btn-wrapper">
                                <a href="myplay.php">
                                    <div class="tab-submit-yellow">
                                        <span class="unskewed">PLAY</span>
                                        <img src="app-assets/images/home/next.png" width="12">
                                       
                                    </div>

                               </a>
                             </div>
                           
                         </div>
                     

                    </div>

        </div>

    </div>

    <table class="table zero-configuration py-2 play">
        <thead>
            <tr>
                <th>Name</th>

            </tr>
        </thead>
        <tbody>

          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                     <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
                    <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
           <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_x_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">160.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_x.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>
          <tr class="py-2">
            <td>
                <div class="row">
                    <div class="col-12 button_right_bg">
                        <div class="row">
                            <div class="col-7">
                                <div class="row">
                                    <div class="col-12">
                                        <h3 style="color:#fff;">   
                                            <img class="img-fluid icon-gp" src="app-assets/images/home/gp.png">
                                            <span class="dollar">130.</span><span class="cent">00</span></h3>
                                            
                                    </div>
                                </div>
                                
                            </div>
                            <div class="col-5 text-right">
                                <div class="row">
                                    <div class="col-12">
                                        <img class="img-fluid action_button" src="app-assets/images/home/button_right.png" width="104">
                                    </div>
                                </div>

   
                            </div>
                             </div>
                            <div class="row">
                                <div class="col-7">
                                    <span class="playing">Playing Block: </span>
                                    <span class="figure">#434217</span>
                                </div>
                                 <div class="col-5 text-right">
                                    <p class="play-time">11-08-2022 7:89 PM</p>
                                </div>
                            </div>
                       
                    </div>
                </div>
            </td>
                

          </tr>




                                               
            </tbody>
            <tfoot>
                <tr>
                    <th>Name</th>
                </tr>
            </tfoot>
     </table>
    </div>

     </div>
</div>



<?php include('footer.php');?>
